...
unsigned int hashKey(string key_string) {
    unsigned int key = 0;
    for (int i = 0; i < (int) key_string.length(); i++) {
        key = key * 31 + key_string[i];
    }
    return key;
}
...
vector<int> generateKey() {
    ...
    for (int i = 0; i < KEY_SIZE; i++) {
        int r = rand() % (KEY_SIZE - i);
        result.push_back(numbers[r]);
        numbers[r] = numbers[(KEY_SIZE - i - 1)];
        numbers[(KEY_SIZE - i - 1)] = result[i];
    }
    return result;
}
...
int checkKey(vector<int> &key) {
    int* check = (int*) malloc(KEY_SIZE * sizeof(int));
    for (int i = 0; i < KEY_SIZE; i++) {
        check[key[i]]++;
    }
    for (int i = 0; i < KEY_SIZE; i++) {
        if (check[i] != 1) {
            printf("Inalid Key at %d\n", i);
